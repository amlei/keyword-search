...
class DouYin:
    ...
    def option_key(self, keyword: str, option: int = 0, last_keyword: str = None) -> str:
        ...
        match option:
            case 0:
                # try:
                    ret = data[0]["Platform"][0][self.platform]['Last Keywords'][keyword]
                # except:
                #     # 当前 keyword 不存在时
                #     pass
            case 1:
                if last_keyword:
                    with open("../file/data.json", "w", encoding="utf-8") as f:
                        data[0]["Platform"][self.platform]['Last Keywords'][keyword] = last_keyword
